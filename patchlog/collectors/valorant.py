...
VALORANT_PATCH_NOTES_URL = "https://playvalorant.com/ko-kr/news/tags/patch-notes/"
VALORANT_BASE_URL = "https://playvalorant.com"
USER_AGENT = "PATCHLOG-RAG/0.1 (+local learning project)"


class ValorantCollector(BaseCollector):
    game = "valorant"
    ...
    def list_patches(
        ...
        *,
        ...
        self._ensure_allowed(VALORANT_PATCH_NOTES_URL)
        response = self.client.get(VALORANT_PATCH_NOTES_URL)
        response.raise_for_status()
        articles = parse_patch_list_html(response.text, base_url=VALORANT_BASE_URL)
        if since:
            articles = [
                article
    ...
    def collect(
        ...
        *,
        ...
        for index, article in enumerate(articles):
            ...
            parsed = parse_valorant_patch_html(collected.html, source_url=article.url)
    ...
    def _ensure_allowed(self, url: str) -> None:
        robots_url = urljoin(VALORANT_BASE_URL, "/robots.txt")
...
def parse_patch_list_html(html: str, *, base_url: str = VALORANT_BASE_URL) -> list[PatchArticle]:
...
def _looks_like_patch_note_url(url: str) -> bool:
    ...
    return (
        "playvalorant.com/ko-kr/news/game-updates/" in normalized
