...
from patchlog.collectors.valorant import extract_patch_version
...
def parse_valorant_patch_html(html: str, *, source_url: str) -> ParsedPatch:
    soup = BeautifulSoup(html, "html.parser")
    title = _extract_title(soup)
    patch_version = extract_patch_version(title) or extract_patch_version(source_url)
    if patch_version is None:
        raise ValueError("Could not extract Valorant patch version from page")

    patch_date = _extract_patch_date(soup)
    if patch_date is None:
        raise ValueError("Could not extract Valorant patch date from page")
...
def _extract_title(soup: BeautifulSoup) -> str:
    ...
    raise ValueError("Could not extract Valorant patch title from page")
...
def _extract_article_body(soup: BeautifulSoup):
    ...
    raise ValueError("Could not extract Valorant patch article body from page")
